...
const config = {
  ...
  fieldsCmb: {
    cmb1: ["Production", "Test"],
    cmb2: ["Active", "Desactivated"],
  },
  closeForm: {
    active: false,
    timeInMs: 300000,
...
const formValidation = async (e) => {
  switch (e.target.name) {
    case "document":
      fieldValidation(expresiones.telefono, e.target, "txtDocument", false);
      break;
    case "name":
    ...
    case "email":
      ...
      let respi = JSON.parse(
        await UC_get_async(
          `SELECT id_customer FROM ccrepo.CRMLite_customers WHERE email = "${e.target.value}" LIMIT 1`,
          ""
        )
      );
      if (respi && respi[0].id_customer != customerFound) {
        swal(
          "We found a customer with this same email",
    ...
    case "phone":
      ...
      let resp = JSON.parse(
        await UC_get_async(
          `SELECT id_customer FROM ccrepo.CRMLite_customers WHERE phone = "${e.target.value}" LIMIT 1`,
          ""
        )
      );
      if (resp && resp[0].id_customer != customerFound) {
        swal(
          "We found a customer with this phone",
...
function manualFormValidation() {
  fieldValidation(
    ...
    document.getElementById("txtDocument"),
    "txtDocument",
    false
  );
  fieldValidation(
  ...
  fieldValidation(
    expresiones.texto,
    document.getElementById("txtColony"),
    "txtColony",
    false
  );
  fieldValidation(
    expresiones.cp,
    document.getElementById("txtCp"),
    "txtCp",
    false
  );
  fieldValidation(
...
document.getElementById("btnSave").addEventListener("click", async () => {
  ...
  if (!hayMasTipificaciones) {
    if (
      !config.saveWithoutDispo ||
      (config.saveWithoutDispo && callid) ||
      !customerFound
    ) {
    ...
    let cliente = await loadCustomer(
  ...
  if (document.getElementById("dateframe").value) {
    ...
    let resp = await UC_exec_async(
      `INSERT INTO ccdata.calls_scheduler (id, calldate, campaign, destination, agentphone) VALUES(null, 
                ...
                '${config.defaultPreview}', '${callerid}', ${
        parent.agent.name
      })`,
      ""
    );
...
document.getElementById("btnClean").addEventListener("click", async () => {
  if (!customerFound) {
    ...
    swal("Warning", "Do you want to delete this record?", "warning", {
      ...
      if (res) {
        let resp = await UC_exec_async(
          `DELETE FROM CRMLite_customers WHERE id_customer = ${customerFound}`,
          ""
        );
...
async function init() {
  ...
  config.fieldsCmb.cmb1.map((item) =>
    $("#cmbField4").append(new Option(item, item))
  );
  $("#cmbField4").trigger("chosen:updated");
  ...
  config.fieldsCmb.cmb2.map((item) =>
    $("#cmbField5").append(new Option(item, item))
  );
  $("#cmbField5").trigger("chosen:updated");
...
async function completarD1() {
  ...
  let resp = await UC_get_async(
    `SELECT distinct value1 FROM ccdata.dispositions WHERE campaign = '${campana}' AND channel = '${canal}'`,
    ""
  );
  ...
  if (respuesta[0].value1.length > 0) {
    ...
    respuesta.map((item) =>
      $("#cmbRes1").append(new Option(item.value1, item.value1))
    );
    $("#cmbRes1").trigger("chosen:updated");
  }
}
...
async function loadCustomer(callid, justAsking = false) {
  //phone or email

  let resp = JSON.parse(
    await UC_get_async(
      `SELECT * FROM ccrepo.CRMLite_customers WHERE id_customer = ${callid} LIMIT 1`,
      ""
    )
  );
  ...
  if (!resp.length) {
    resp = JSON.parse(
      await UC_get_async(
        `SELECT * FROM ccrepo.CRMLite_customers WHERE phone = "${callid}" LIMIT 1`,
        ""
      )
    );
  }
  ...
  if (!resp.length) {
    resp = JSON.parse(
      await UC_get_async(
        `SELECT * FROM ccrepo.CRMLite_customers WHERE email = "${callid}" LIMIT 1`,
        ""
      )
    );
  }
  ...
  if (resp.length > 0) {
    ...
    resp[0].field4
      ? (document.getElementById("cmbField4").value = resp[0].field4)
      : null;
    resp[0].field5
      ? (document.getElementById("cmbField5").value = resp[0].field5)
      : null;
