...
function manualFormValidation() {
  ...
  fieldValidation(expresiones.telefono, document.getElementById('txtDocument'), 'txtDocument', false);
  ...
  if (!customerFound) {
    ...
    swal('Warning', 'Do you want to delete this record?', 'warning', {
      ...
      if (res) {
        ...
        if (resp == "OK") {
          swal('Congrats', 'The customers has been delete successfully', 'success');
          reactform.reset();
          UC_closeForm();
        } else {
          swal('Ups!', 'We have some issues exprience, please try later', 'warning');
        }
      }
...
document.getElementById('phoneAddon').addEventListener('click', () => {
  if (!document.getElementById('txtPhone').value) {
    notification('Sorry', 'the phone number is empty', 'fa fa-warning', 'warning');
  } else {
